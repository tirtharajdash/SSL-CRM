...
def change_data_to_assumption(X_train, test_dataset, num_neurons):
    """
    Change the data according to the assumption that all the neurons after the id 'num_neurons' have f-value of 1.
    """
    for x in (X_train):
...
def make_dataset_cli(
    ...
    for i in range(len(edges_raw)):
        ...
        if b_match:
            ...
            print(f"u_match: {u_match}")
            print(f"edges_raw[{i}]: {edges_raw[i]}")
            quit()
    ...
    if os.path.exists(f"{train_pos_file}"):
        with open(f"{train_pos_file}", "r") as f:
            while True:
                ...
                dd = {i: 1 if i in all_pos else 0 for i in range(num_neurons)} #Added "or i >= 5050" for assumption, remove later in 4 places *****
                X_train.append(dd)
