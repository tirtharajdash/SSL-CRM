...
%                                                                         %
% A L E P H                                                               %
% Version 6      (last modified: Sat Nov 25 02:54:21 UTC 2017)            %
%                                                                         %
...
aleph_version(6).
aleph_version_date('Sat Nov 25 02:54:21 UTC 2017').
...
set_counter(N):-
	retract('$aleph_local'(counter,_)), !,
	asserta('$aleph_local'(counter,N)).
set_counter(N):-
	asserta('$aleph_local'(counter,N)).

get_counter(N):-
	'$aleph_local'(counter,N).

inc_counter(Inc):-
	retract('$aleph_local'(counter,N)), !,
	N1 is N + Inc,
	asserta('$aleph_local'(counter,N1)).
inc_counter(Inc):-
	asserta('$aleph_local'(counter,Inc)).
...
read_all(Prefix):-
	read_all(Prefix,Prefix,Prefix,Prefix).
...
read_all(Back,Pos,Neg,Rand):-
	clean_up,
	reset,
	read_background(Back),
	read_examples(Pos,Neg,Rand), 	
	record_targetpred, 	
	check_recursive_calls,
	check_prune_defs,
	check_user_search,
...
read_examples(Pos,Neg,Rand):-
	...
	(setting(rand,RandF) ->
		set(use_file_extensions,false),
		read_examples_files(rand,RandF,_),
		noset(use_file_extensions);
		read_examples_files(rand,Rand,RandF),
		set(rand,RandF)
	),
...
file_extension(rand,Suffix):- name('.r',Suffix).
...
gen_feature(Feature,Label,Class):-
	...
	aleph_assert('$aleph_feature'(Id,Label,Class,Template,Body)),
	aleph_assert('$aleph_feature_weight'(Id,1.0)).
