...
% :- modeb(*,symbond(+mol,+atomid,-atomid,#bondtype)).
% :- modeb(*,bond(+mol,-atomid_1,-atomid_2,#element,#element,#bondtype)).
:- modeb(*,bond(+mol,-atomid,-atomid,#element,#element,#bondtype)).
...
:- modeb(1,gteq(+length,#num)).
:- modeb(1,lteq(+length,#num)).

:- modeb(1,(+atomids = +atomids)).
