...
def main():
    ...
    #print(f"number of output nodes: {len([a for a in adj_list if a == []])}")
    # Create CRM structure and train with input data
    print("***Creating CRM structure*** ")
    #n = Network(len(adj_list), adj_list)
    n = CRM_model(len(adj_list), adj_list, ajd_matrix=False, output_layer=True)
    n.to(device)
    ...
    assumption = True
    if assumption:
        print("***Changing data to assumption***")
        change_data_to_assumption(X_train, test_dataset, sum([len(neurons) for neurons in n.layers_list[:1]])) # change the number to layer after which all f-value are 1
...
#    train_losses, val_losses = train(
    ...
    # # train metrics for autoencoder
    # print("***Train Metrics***")
    # print(get_autoencoder_metrics(n, X_train, X_train))
